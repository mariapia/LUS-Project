...
while read -r line
do
	ngramcount --order=$1 --require_symbols=false tagsentence.far > pos_$2$1.cnt
	ngrammake --method=$2 pos_$2$1.cnt > pos_$2$1.lm
	...
	fstcompose 1.fst $automa | fstcompose - pos_$2$1.lm | fstrmepsilon | fstshortestpath | fsttopsort | fstprint --isymbols=$lexicon --osymbols=$lexicon >> $2/$output
	echo " " >> $2/$output
	((sentence_counter++))
...
rm tmp.txt pos_$2$1* 1.fst tagsentence.far 
